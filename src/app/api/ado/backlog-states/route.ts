...
export async function GET(req: Request) {
  ...
  try {
    const states = await listBacklogItemStates(authForProject);
    return NextResponse.json({
      states,
      workItemType: process.env.AZDO_BACKLOG_ITEM_TYPE?.trim() || "Product Backlog Item",
    });
  } catch (cause) {
    const detail = cause instanceof Error ? cause.message : "Error desconocido";
    return NextResponse.json(
      { error: "No se pudieron cargar los estados del backlog.", detail },
      { status: 502 },
    );
  }
}
